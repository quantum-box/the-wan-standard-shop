...
test("storefront product loading rejects GraphQL errors instead of treating them as empty products", async () => {
  const originalFetch = globalThis.fetch;
...
test("storefront product loading rejects HTTP errors", async () => {
  ...
  try { await expect(getProducts()).rejects.toThrow("GraphQL API error: 503"); }
  finally { globalThis.fetch = originalFetch; }
});
