...
export async function createOnlinePickupOrder(input: {
  ...
  if (!payload.data?.checkout.checkoutUrl) {
    throw new Error("Online checkout URL is missing");
  }
  return {
    id: payload.data.checkout.id,
    checkoutUrl: payload.data.checkout.checkoutUrl,
  };
}
