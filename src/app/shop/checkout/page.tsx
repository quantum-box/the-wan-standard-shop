...
export default function CheckoutPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      else router.push(`/shop/checkout/thanks?order=${result.id}`);
    } catch {
  ...
  return (
    <div className="max-w-lg">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        ...
        <p className="text-xs text-n1 leading-relaxed">
          注文を確定する前に、<Link href="/guide/cancel" className="underline text-p2">返品・キャンセルポリシー</Link>をご確認ください。
        </p>
