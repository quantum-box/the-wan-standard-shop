...
export default function CheckoutPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      saveOrderReceipt({ orderId: result.id, cart, name: form.name, phone: form.phone, email: form.email || undefined, createdAt: createdAt.toISOString(), pickupDeadline: deadline.toISOString() });
      clearCartId();
      if (result.checkoutUrl) window.location.href = result.checkoutUrl;
      else router.push(`/shop/checkout/thanks?order=${encodeURIComponent(result.id)}`);
  ...
  return <div className="max-w-lg">
    ...
    <div className="border border-s2/40 bg-white px-4 py-3 mb-6 text-sm text-p2"><p className="font-medium mb-1">店舗受け取り</p><p className="text-n1 text-xs">バーナードスクエア ドッグラン施設にてお受け取りください。</p><p className="text-n1 text-xs mt-1">お支払いは店頭にてお願いいたします。</p></div>
