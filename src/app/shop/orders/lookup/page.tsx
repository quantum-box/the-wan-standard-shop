...
export default function OrderLookupPage() {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const result = await getOrderByLookup({ phone, lastDigits });
  ...
  return <div className="max-w-xl">
    ...
    <p className="text-sm text-n1 mb-8">ご注文時の電話番号と注文番号の下4桁で、店舗受け取り注文を確認できます。</p>
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      <div><label className="block text-xs text-n1 mb-1">電話番号</label><input value={phone} onChange={(e)=>setPhone(e.target.value)} required type="tel" inputMode="tel" placeholder="090-1234-5678" className="w-full border border-s2/60 px-3 py-2 text-sm bg-p1 text-p2 focus:outline-none focus:border-p2" /></div>
      <div><label className="block text-xs text-n1 mb-1">注文番号の下4桁</label><input value={lastDigits} onChange={(e)=>setLastDigits(e.target.value)} required minLength={4} maxLength={32} placeholder="A1B2" className="w-full border border-s2/60 px-3 py-2 text-sm bg-p1 text-p2 uppercase focus:outline-none focus:border-p2" /></div>
