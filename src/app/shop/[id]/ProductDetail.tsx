...
export function ProductDetail({ productId }: Props) {
  ...
  async function handleAddToCart() { if (!product || product.stock <= 0) return; setAdding(true); try { const cart = await addToCart(getCartId(), product.id, quantity); setCartId(cart.id); router.push("/shop/cart"); } catch { alert("カートへの追加に失敗しました。再度お試しください。"); } finally { setAdding(false); } }
